...
/// Timing repetitions per case; the median is reported. 5 reps keep
/// the median stable on shared runners, where single-shot scheduling
/// jitter on microsecond-scale cases swings ±20% (observed as false
/// Rule 41 trips on rt_wide8 at 3 reps).
...
struct InterpCtx {
    std::vector<std::pair<SymbolId, const ASTFnDecl*>> fns;
};

int interp_stmts(const ASTNode& n, std::vector<int64_t>& env,
                 std::vector<SymbolId>& names, const InterpCtx& ctx,
...
[[nodiscard]] int64_t interp_expr_sym(const ASTNode& n,
    ...
    switch (n.kind) {
        ...
        case ASTKind::BinaryExpr: {
            ...
            const int64_t l = interp_expr_sym(*b.lhs, env, names, ctx);
            const int64_t r = interp_expr_sym(*b.rhs, env, names, ctx);
            switch (b.op) {
        ...
        case ASTKind::UnaryExpr: {
            ...
            const int64_t v = interp_expr_sym(*u.operand, env, names, ctx);
        ...
        case ASTKind::CallExpr: {
            ...
            if (!c.callee || c.callee->kind != ASTKind::Ident ||
                c.args.size() > 4) {
                std::fprintf(stderr, "interp: unsupported call shape\n");
                std::abort();
            }
            const SymbolId callee =
                static_cast<const ASTIdent&>(*c.callee).name;
            const ASTFnDecl* callee_fn = nullptr;
            for (const auto& [sym, decl] : ctx.fns) {
                if (sym == callee) { callee_fn = decl; break; }
            }
            if (callee_fn == nullptr) {
                std::fprintf(stderr, "interp: unknown callee\n");
                std::abort();
            }
            std::vector<int64_t> cenv;
            for (const auto& a : c.args) {
                cenv.push_back(
                    interp_expr_sym(*a, env, names, ctx));
            }
            std::vector<SymbolId> cnames;
            for (const auto& pp : callee_fn->params) {
                cnames.push_back(static_cast<const ASTParam&>(*pp).name);
            }
            cenv.resize(cnames.size(), 0);
            int64_t value = 0;
            bool exited = false;
            interp_stmts(*callee_fn->body, cenv, cnames, ctx, value,
                         exited);
            if (!exited) {
                std::fprintf(stderr, "interp: callee never returned\n");
                std::abort();
            }
            return value;
        }
    ...
    switch (n.kind) {
        case ASTKind::Block: {
            ...
            for (const auto& s : b.stmts) {
                if (!s) continue;
                if (interp_stmts(*s, env, names, ctx, exit_value, exited) != 0)
                    return 1;
            }
            return 0;
        }
        case ASTKind::LetStmt: {
            ...
            int64_t v = 0;
            if (s.init) v = interp_expr_sym(*s.init, env, names, ctx);
            for (size_t k = 0; k < names.size(); ++k) {
                if (names[k] == s.name) { env[k] = v; return 0; }
            }
            // Callee bodies bind names the caller never registered
            // (only the ENTRY function's bindings are pre-registered):
            // register dynamically at the current frame's tail.
        ...
        case ASTKind::AssignExpr: {
            ...
            const int64_t v = interp_expr_sym(*a.value, env, names, ctx);
        ...
        case ASTKind::IfStmt: {
            ...
            const int64_t c = interp_expr_sym(*s.cond, env, names, ctx);
            const ASTNode* arm = c != 0 ? s.then_branch.get()
                                        : s.else_branch.get();
            if (arm) return interp_stmts(*arm, env, names, ctx, exit_value, exited);
            return 0;
        }
        case ASTKind::ForStmt: {
            ...
            const int64_t lo = interp_expr_sym(*range.lo, env, names, ctx);
            const int64_t hi = interp_expr_sym(*range.hi, env, names, ctx);
            names.push_back(s.var_name);
            env.push_back(0);
            for (int64_t i = lo; i < hi; ++i) {
                env.back() = i;
                if (interp_stmts(*s.body, env, names, ctx, exit_value,
                                 exited) != 0)
                    break;
            }
            names.pop_back();
        ...
        case ASTKind::ReturnStmt: {
            ...
            exit_value = s.value
                ? interp_expr_sym(*s.value, env, names, ctx) : 0;
            exited = true;
            return 1;
        }
        default:
...
struct CompiledCase {
    std::string name;
    std::vector<uint8_t> code{};         // LINKED module image
    size_t entry_offset{0};              // entry fn's offset in image
    ...
    size_t arity{0};
    size_t ir_nodes{0};
    // All functions (for the reference interpreter's CallExpr).
    std::vector<std::pair<SymbolId, const ASTFnDecl*>> fns{};
    const ASTFnDecl* entry_fn{nullptr};
};
...
[[nodiscard]] bool compile_case(const std::string& name,
    ...
    // OWN the AST before anything takes pointers into it (the entry
    // scan below reads through out.module).
    out.module = std::move(mod.value());
    // Entry = `main` when present, else the first function (single-fn
    // cases have no main). Call cases name their entry main.
    const ASTFnDecl* entry = nullptr;
    for (const auto& item : out.module->items) {
        if (!item || item->kind != ASTKind::FnDecl) continue;
        const auto& f = static_cast<const ASTFnDecl&>(*item);
        if (syms.at(f.name) == "main") { entry = &f; break; }
    }
    if (entry == nullptr) {
        for (const auto& item : out.module->items) {
            if (item && item->kind == ASTKind::FnDecl) {
                entry = static_cast<const ASTFnDecl*>(item.get());
                break;
            }
        }
    }
    if (entry == nullptr) { err = name + ": no functions"; return false; }
    const auto& fn = *entry;
    ...
    // ---- Per-function compilation (separate graphs; ONE symbol
    // table so call sites and callees agree on ids), then module
    // linking. Single-function cases ride the same path (a 1-fn
    // module with no calls links to itself trivially).
    struct FnState {
        SymbolId symbol;
        Graph graph;                 // owns the fn's IR
        MachineFunction mf;
    };
    std::vector<FnState> fn_states;
    for (const auto& item : out.module->items) {
        if (!item || item->kind != ASTKind::FnDecl) continue;
        const auto& f = static_cast<const ASTFnDecl&>(*item);
        out.fns.emplace_back(f.name, &f);
        FnState st{f.name, Graph(&syms), MachineFunction{}};
        Lowerer lw(st.graph, &syms);
        if (!lw.lower_fn(f).has_value()) {
        ...
        std::string why;
        if (!st.graph.verify(why)) {
            err = name + ": verify failed: " + why; return false;
        }
    ...
    // Register allocation per function (call-aware: caller-saved
    // homes excluded for call-spanning intervals).
    std::deque<LinearScanAllocator> ras;
    std::vector<backend::x86::ModuleFunction> mods;
    for (auto& st : fn_states) {
        ras.emplace_back(st.mf, backend::x86::kExecHomeRegCount, 0);
        ras.back().set_callee_saved_from(
            backend::x86::kExecFirstCalleeSaved);
        const uint32_t spills = ras.back().run();
        if (spills != 0) {
            err = name + ": " + std::to_string(spills) +
                  " spills in fn " + syms.at(st.symbol).data() +
                  " — executable path requires zero spills";
            return false;
        }
        // deque: &ras.back() stays valid as more are appended.
        mods.push_back(
            backend::x86::ModuleFunction{st.symbol, &st.mf, &ras.back()});
    }

    // ---- No-silent-omission guard, PER FUNCTION (Rule D.3). ----
    for (auto& st : fn_states) {
        const MachineFunction& mf = st.mf;
        VRegId max_v = 0;
        ...
        for (const auto& mi : mf.instrs) {
            ...
            break; // single ret per function in the corpus
        }
    }

    // ---- Link. ----
    std::vector<size_t> offsets;
    if (!backend::x86::encode_module(mods, out.code, offsets, err)) {
        err = name + ": link failed: " + err;
        return false;
    }
    // Entry = `main` when present, else the single function.
    {
        SymbolId entry_sym = kInvalidSymbolId;
        for (const auto& [sym, decl] : out.fns) {
            if (syms.at(sym) == "main") { entry_sym = sym; break; }
        }
        size_t idx = 0;
        for (size_t i = 0; i < fn_states.size(); ++i) {
            if (fn_states[i].symbol == entry_sym ||
                (entry_sym == kInvalidSymbolId && i == 0)) {
                idx = i;
                break;
            }
        }
        out.entry_offset = offsets[idx];
        out.entry_fn = out.fns[idx].second;
    }
    // (The no-silent-omission guard runs PER FUNCTION before
    // linking — see the module-mode compile path above.)
    out.name = name;
    return true;
}
...
[[nodiscard]] std::vector<CaseSpec> build_corpus() {
    return {
        ...
        {"rt_call_simple",
         ...
         {{0}, {1}, {4}, {7}, {-3}}},
        {"rt_call_args3",
         "fn mix3(a: i32, b: i32, c: i32) -> i32 { return a * 7 + b * 11 + c * 13; }\n"
         "fn main(n: i32) -> i32 { return mix3(n, n + 1, 2 * n) + mix3(n + 2, 1, n - 1); }\n",
         {{0}, {1}, {3}, {5}, {-4}}},
        {"rt_call_loop",
...
int main() {
    ...
    for (const auto& spec : corpus) {
        ...
        for (const auto& args : spec.inputs) {
            ...
            auto* entry_ptr = static_cast<char*>(page) + cc.entry_offset;
            const int64_t got = call_compiled(entry_ptr, cc.arity, args);
            int64_t want;
            if (cc.statement_mode) {
                ...
                bool exited = false;
                interp_stmts(*cc.body_block, env, cc.param_syms, ictx,
                             want, exited);
                if (!exited) {
                ...
                want = interp_expr_sym(*cc.body_expr, args, cc.param_syms, ictx);
            }
            if (got != want) {
                std::fprintf(stderr,
        ...
        for (int rep = 0; rep < kTimingReps; ++rep) {
            ...
            for (int64_t i = 0; i < kCallsPerCase; ++i) {
                ...
                checksum_code +=
                    call_compiled(static_cast<char*>(page) + cc.entry_offset,
                                  cc.arity, args);
            }
            ...
            for (int64_t i = 0; i < kCallsPerCase; ++i) {
                ...
                if (cc.statement_mode) {
                    ...
                    iv = interp_expr_sym(*cc.body_expr, args, cc.param_syms, ictx);
                }
                checksum_interp += iv;
            }
